...
public class MouseTracker {
	...
	public void update() {
		...
		x=m.getScaledXPos(window);
		y=m.getScaledYPos(window);
		isCursorLocked=m.isMouseGrabbed();

		boolean left=GLFW.glfwGetMouseButton(window.handle(),GLFW.GLFW_MOUSE_BUTTON_LEFT)==GLFW.GLFW_PRESS;
		leftClicked=left&&!isLeftDown;
		isLeftDown=left;

		boolean middle=GLFW.glfwGetMouseButton(window.handle(),GLFW.GLFW_MOUSE_BUTTON_MIDDLE)==GLFW.GLFW_PRESS;
		middleClicked=middle&&!isMiddleDown;
		isMiddleDown=middle;

		boolean right=GLFW.glfwGetMouseButton(window.handle(),GLFW.GLFW_MOUSE_BUTTON_RIGHT)==GLFW.GLFW_PRESS;
